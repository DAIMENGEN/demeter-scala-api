...
trait DBTableRowFactory {
   ...
   *
   * @param userId The user ID associated with the row data.
   * @param entityData The entity data used to create the row data.
   * @return A new instance of `TableRowData`.
   */
  def create(userId: Long, entityData: EntityData): TableRowData
   ...
   */
  def update(userId: Long, entityData: EntityData, oldRowData: TableRowData): TableRowData
}
