...
import com.advantest.demeter.core.database.project.{ProjectDBTableRow, ProjectDBTableSchema}
 ...
 */
final class ProjectPermissionDBTableSchema(tag: Tag) extends Table[ProjectPermissionDBTableRow](tag, "PROJECT_PERMISSION_DB_TABLE") with DBTableSchema {
  ...
  override def * : ProvenShape[ProjectPermissionDBTableRow] = (
    id,
    category,
    permission,
    employeeId,
    ...
    createDateTime,
    updateDateTime()
  ) <> ((ProjectPermissionDBTableRow.apply _).tupled, ProjectPermissionDBTableRow.unapply)
}
