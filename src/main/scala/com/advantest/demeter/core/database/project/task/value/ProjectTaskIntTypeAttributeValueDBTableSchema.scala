...
final class ProjectTaskIntTypeAttributeValueDBTableSchema(tag: Tag) extends ProjectTaskAttributeValueDBTableSchema[ProjectTaskAttributeValueDBTableRow](tag, "PROJECT_TASK_INT_TYPE_ATTRIBUTE_VALUE_DB_TABLE") {
  ...
  def attributeValue: Rep[DBIntValue] = column[DBIntValue]("ATTRIBUTE_VALUE")
  ...
  override def * : ProvenShape[ProjectTaskAttributeValueDBTableRow] = (
    ...
    (row: ProjectTaskAttributeValueDBTableRow) => row.attributeValue match {
      case attributeValue: DBIntValue => Some((row.id, row.attributeId, attributeValue, row.projectId, row.creatorId, row.updaterId, row.createDateTime, row.updateDateTime))
      case _ => None
    }
  )
}
