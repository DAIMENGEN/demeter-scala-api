...
import com.advantest.demeter.utils.database.DBDoubleTypeValue
...
final class ProjectTaskDoubleTypeAttributeValueDBTableSchema(tag: Tag) extends ProjectTaskAttributeValueDBTableSchema[ProjectTaskAttributeValueDBTableRow](tag, "PROJECT_TASK_DOUBLE_TYPE_ATTRIBUTE_VALUE_DB_TABLE") {
  ...
  override def taskAttributeValue: Rep[DBDoubleTypeValue] = column[DBDoubleTypeValue]("TASK_ATTRIBUTE_VALUE")
  ...
  override def * : ProvenShape[ProjectTaskAttributeValueDBTableRow] = (
    ...
    (row: ProjectTaskAttributeValueDBTableRow) => row.taskAttributeValue match {
      case taskAttributeValue: DBDoubleTypeValue => Some((row.id, row.taskId, row.taskAttributeId, taskAttributeValue, row.projectId, row.creatorId, row.updaterId, row.createDateTime, row.updateDateTime))
      case _ => None
    }
  )
}
