...
final class ProjectTaskTextTypeAttributeValueDBTableSchema(tag: Tag) extends ProjectTaskAttributeValueDBTableSchema[ProjectTaskAttributeValueDBTableRow](tag, "PROJECT_TASK_TEXT_TYPE_ATTRIBUTE_VALUE_DB_TABLE") {
  ...
  def attributeValue: Rep[DBTextValue] = column[DBTextValue]("ATTRIBUTE_VALUE", O.SqlType("TEXT"))
  ...
  override def * : ProvenShape[ProjectTaskAttributeValueDBTableRow] = (
    ...
    (row: ProjectTaskAttributeValueDBTableRow) => row.attributeValue match {
      case attributeValue: DBTextValue => Some((row.id, row.attributeId, attributeValue, row.projectId, row.creatorId, row.updaterId, row.createDateTime, row.updateDateTime))
      case _ => None
    }
  )
}
