...
import com.advantest.demeter.utils.database.DBLongTypeValue
...
final class ProjectTaskLongTypeAttributeValueDBTableSchema(tag: Tag) extends ProjectTaskAttributeValueDBTableSchema[ProjectTaskAttributeValueDBTableRow](tag, "PROJECT_TASK_LONG_TYPE_ATTRIBUTE_VALUE_DB_TABLE") {
  ...
  override def taskAttributeValue: Rep[DBLongTypeValue] = column[DBLongTypeValue]("TASK_ATTRIBUTE_VALUE")
  ...
  override def * : ProvenShape[ProjectTaskAttributeValueDBTableRow] = (
    ...
    (row: ProjectTaskAttributeValueDBTableRow) => row.taskAttributeValue match {
      case taskAttributeValue: DBLongTypeValue => Some((row.id, row.taskId, row.taskAttributeId, taskAttributeValue, row.projectId, row.creatorId, row.updaterId, row.createDateTime, row.updateDateTime))
      case _ => None
    }
  )
}
