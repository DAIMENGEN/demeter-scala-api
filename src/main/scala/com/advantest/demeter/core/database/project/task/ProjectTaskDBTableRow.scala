...
final case class ProjectTaskDBTableRow(
  ...
  override def toString: String = s"ProjectTaskDBTableRow(id=$id, taskName=$taskName, order=$order, creatorId=$creatorId, updaterId=$updaterId, createDateTime=$createDateTime, updateDateTime=$updateDateTime)"
}
