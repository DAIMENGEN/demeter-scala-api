...
import com.advantest.demeter.utils.database.DBStringTypeValue
...
final class ProjectTaskStringTypeAttributeValueDBTableSchema(tag: Tag) extends ProjectTaskAttributeValueDBTableSchema[ProjectTaskAttributeValueDBTableRow](tag, "PROJECT_TASK_STRING_TYPE_ATTRIBUTE_VALUE_DB_TABLE") {
  ...
  override def taskAttributeValue: Rep[DBStringTypeValue] = column[DBStringTypeValue]("TASK_ATTRIBUTE_VALUE", O.Length(255))
  ...
  override def * : ProvenShape[ProjectTaskAttributeValueDBTableRow] = (
    ...
    (row: ProjectTaskAttributeValueDBTableRow) => row.taskAttributeValue match {
      case taskAttributeValue: DBStringTypeValue => Some((row.id, row.taskId, row.taskAttributeId, taskAttributeValue, row.projectId, row.creatorId, row.updaterId, row.createDateTime, row.updateDateTime))
      case _ => None
    }
  )
}
