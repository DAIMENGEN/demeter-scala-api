...
case class EmployeeService()(implicit val db: Database) extends Service {
  ...
  def getEmployeeSelectOptions(employeeName: String): Future[Seq[SelectOption]] = {
    employeeTable.queryByEmployeeNameLike(employeeName).map(employees => employees.map(employee => SelectOption(employee.employeeName.value, LongValue(employee.id.value))))
  }
}
